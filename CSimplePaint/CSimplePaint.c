...
int shape_gui() {
	...
	for(int i=0;i<numshapes;i++) {
		...
		box(x1,x2,w+4,h+4,colors[b*3+0],colors[b*3+1],colors[b*3+2]);
		box(x1,x2,w,h,colors[b*3+0],colors[b*3+1],colors[b*3+2]);
		switch(i) {
			case 0:
				line(x1,y2,x2,y2,colors[b*3+0],colors[b*3+1],colors[b*3+2]);
			break;
			case 1:
				circle(i*(sz+g*2)+g-2+w/2,sz+g-2+h/2,sz/2,colors[b*3+0],colors[b*3+1],colors[b*3+2]);
			break;
			case 2:
				box(x1,y2,w+4,h+4,colors[b*3+0],colors[b*3+1],colors[b*3+2]); break;
		}
	}
	...
	while(!quit) {
		...
		if(!hold) {
			if(mouseleftdown) {
				...
				switch(shapetype) {
					case 0:
						line(mouse_x-sz,mouse_y-10,mouse_x+sz,mouse_y+sz,color_r,color_g,color_b);
					break;
					case 1:
						fill_circle(mouse_x,mouse_y,sz,color_r,color_g,color_b);
					break;
					case 2:
						fill_box(mouse_x-sz/2,mouse_y-sz/2,sz,sz,color_r,color_g,color_b);
					break;
					default: break;
				}
			}
